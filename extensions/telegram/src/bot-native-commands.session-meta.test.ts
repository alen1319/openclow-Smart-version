...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  it("keeps native command authorization subject sender-scoped in forum topics", async () => {
    const { handler } = registerAndResolveStatusHandler({
      cfg: {},
      allowFrom: ["200"],
      groupAllowFrom: ["200"],
    ...
    const dispatchCall = (
      replyMocks.dispatchReplyWithBufferedBlockDispatcher.mock.calls as unknown as Array<
        [{ ctx?: { AuthorizationSubjectKey?: string } }]
      >
    )[0]?.[0];
    expect(dispatchCall?.ctx?.AuthorizationSubjectKey).toBe("telegram:default:sender:200");
    expect(dispatchCall?.ctx?.AuthorizationSubjectKey).not.toContain("-1001234567890");
  });

  it("keeps native DM authorization subject sender-scoped when chatId differs from senderId", async () => {
    const { handler } = registerAndResolveStatusHandler({
      cfg: {},
      allowFrom: ["200"],
    });
    ...
    expect(dispatchCall?.ctx?.AuthorizationSubjectKey).toBe("telegram:default:sender:200");
    expect(dispatchCall?.ctx?.AuthorizationSubjectKey).not.toContain("61560001");
  });
