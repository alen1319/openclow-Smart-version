...
function dedupeSessionKeys(values: Array<string | undefined | null>): string[] {
...
export function resolveTelegramAuthorizationSenderId(params: {
  chatId: number | string;
  isGroup: boolean;
  senderId?: string | number | null;
  /** When true for DMs, never fall back to chatId; require a direct sender id. */
  strictDirectSender?: boolean;
}): string | undefined {
  const directCandidate = resolveTelegramDirectPeerId({
    chatId: params.chatId,
    senderId: params.senderId,
  }).trim();
  if (params.isGroup) {
    const senderCandidate = String(params.senderId ?? "").trim();
    if (!senderCandidate || senderCandidate.startsWith("-")) {
      return undefined;
    }
    return senderCandidate;
  }
  if (params.strictDirectSender === true) {
    const senderCandidate = String(params.senderId ?? "").trim();
    if (!senderCandidate || senderCandidate.startsWith("-")) {
      return undefined;
    }
  ...
  if (!directCandidate || directCandidate.startsWith("-")) {
    return undefined;
  }
...
export function resolveTelegramConversationLineage(params: {
  cfg: OpenClawConfig;
  route: Pick<
    ReturnType<typeof resolveTelegramConversationRoute>["route"],
    "agentId" | "accountId" | "sessionKey"
  >;
  chatId: number | string;
  isGroup: boolean;
  senderId?: string | number | null;
  ...
  const conversationRootSessionKey = buildAgentSessionKey({
    agentId: params.route.agentId,
    channel: "telegram",
    accountId: params.route.accountId,
    peer: {
      kind: params.isGroup ? "group" : "direct",
      id: params.isGroup
        ? buildTelegramGroupPeerId(params.chatId)
        : resolveTelegramDirectPeerId({
            chatId: params.chatId,
            senderId: params.senderId,
          }),
  ...
  const hasThreadParent = params.resolvedThreadId != null || params.dmThreadId != null;
  const parentSessionKey = hasThreadParent ? conversationRootSessionKey : undefined;
  return {
    parentSessionKey,
    memoryRootSessionKey: conversationRootSessionKey,
    memorySessionKeyCandidates: dedupeSessionKeys([
      params.route.sessionKey,
      parentSessionKey,
      conversationRootSessionKey,
    ]),
  };
}
