...
describe("createTelegramBot", () => {
  ...
  it("keeps group approval callbacks sender-scoped for authorization subject tracking", async () => {
    onSpy.mockClear();
    editMessageReplyMarkupSpy.mockClear();
    resolveExecApprovalSpy.mockClear();
    ...
    expect(editMessageReplyMarkupSpy).toHaveBeenCalledTimes(1);
    expect(answerCallbackQuerySpy).toHaveBeenCalledWith("cbq-approve-group-subject");
  });

  it("keeps forum-topic approval callbacks sender-scoped and approver-scoped", async () => {
    onSpy.mockClear();
    editMessageReplyMarkupSpy.mockClear();
    resolveExecApprovalSpy.mockClear();
  ...
  it("does not treat DM chat ids as approval identities when sender id metadata is missing", async () => {
    onSpy.mockClear();
    editMessageReplyMarkupSpy.mockClear();
    editMessageTextSpy.mockClear();
    resolveExecApprovalSpy.mockClear();
    ...
    await callbackHandler({
      callbackQuery: {
        id: "cbq-approve-missing-sender-id",
        data: "/approve 138e9b8c allow-once",
        from: { first_name: "Ada", username: "ada_bot" },
        message: {
          chat: { id: 1234, type: "private" },
          date: 1736380800,
          message_id: 26,
    ...
    expect(editMessageTextSpy).not.toHaveBeenCalled();
    expect(resolveExecApprovalSpy).not.toHaveBeenCalled();
    expect(answerCallbackQuerySpy).toHaveBeenCalledWith("cbq-approve-missing-sender-id");
  });
  ...
  it("keeps target-only forum-topic approval callbacks sender-scoped and approver-scoped", async () => {
    onSpy.mockClear();
    editMessageReplyMarkupSpy.mockClear();
    editMessageTextSpy.mockClear();
    resolveExecApprovalSpy.mockClear();
    ...
    expect(editMessageReplyMarkupSpy).toHaveBeenCalledTimes(1);
    expect(answerCallbackQuerySpy).toHaveBeenCalledWith("cbq-approve-target-topic");
  });
