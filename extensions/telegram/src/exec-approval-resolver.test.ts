...
describe("resolveTelegramExecApproval", () => {
  ...
  it("injects authorizationSubjectKey into gateway client identity for approval audit trails", async () => {
    const { resolveTelegramExecApproval } = await import("./exec-approval-resolver.js");
    ...
    expect(gatewayRuntimeHoisted.createClientSpy).toHaveBeenCalledWith(
      expect.objectContaining({
        clientDisplayName: expect.stringContaining("subject=telegram:default:sender:9"),
      }),
    );
  });

  it("derives an account-scoped subject key when explicit authorizationSubjectKey is absent", async () => {
    const { resolveTelegramExecApproval } = await import("./exec-approval-resolver.js");
