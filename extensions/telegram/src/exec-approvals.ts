...
export type TelegramAuthorizationActor = {
  accountId: string;
  senderId?: string;
  authorizationSubjectKey?: string;
  /**
   * Stable approver identity for audit trails. For Telegram this intentionally
   * mirrors the sender-scoped authorization subject and never uses chat/group ids.
   */
  approverIdentityKey?: string;
};

export function resolveTelegramAuthorizationActor(params: {
  accountId?: string | null;
  senderId?: string | number | null;
}): TelegramAuthorizationActor {
  const senderId = normalizeTelegramDirectApproverId(params.senderId ?? "");
  const accountId = normalizeAccountId(params.accountId);
  const authorizationSubjectKey = senderId ? `telegram:${accountId}:sender:${senderId}` : undefined;
  return {
    accountId,
    senderId: senderId ?? undefined,
    authorizationSubjectKey,
    approverIdentityKey: authorizationSubjectKey,
  };
}

export function resolveTelegramAuthorizationSubjectKey(params: {
  accountId?: string | null;
  senderId?: string | number | null;
}): string | undefined {
  return resolveTelegramAuthorizationActor(params).authorizationSubjectKey;
}

export function resolveTelegramApproverIdentityKey(params: {
  accountId?: string | null;
  senderId?: string | number | null;
}): string | undefined {
  return resolveTelegramAuthorizationActor(params).approverIdentityKey;
}
