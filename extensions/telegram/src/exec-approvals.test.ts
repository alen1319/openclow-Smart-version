...
import {
  ...
  resolveTelegramAuthorizationActor,
  resolveTelegramApproverIdentityKey,
...
describe("telegram exec approvals", () => {
  ...
  it("builds sender-scoped authorization actors without ever treating group ids as approvers", () => {
    expect(resolveTelegramAuthorizationActor({ accountId: "work", senderId: "9" })).toEqual({
      accountId: "work",
      senderId: "9",
      authorizationSubjectKey: "telegram:work:sender:9",
      approverIdentityKey: "telegram:work:sender:9",
    });
    expect(resolveTelegramAuthorizationActor({ accountId: "work", senderId: "-100999" })).toEqual(
      {
        accountId: "work",
        senderId: undefined,
        authorizationSubjectKey: undefined,
        approverIdentityKey: undefined,
      },
    );
    expect(resolveTelegramApproverIdentityKey({ accountId: "work", senderId: "9" })).toBe(
      "telegram:work:sender:9",
    );
  });

  it("never derives approver identities from topic/group routing ids", () => {
    expect(
      resolveTelegramAuthorizationActor({
        accountId: "work",
        senderId: "-100999:topic:42",
      }),
    ).toEqual({
      accountId: "work",
      senderId: undefined,
