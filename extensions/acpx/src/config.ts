...
const BUNDLED_RUNTIME_DIR_NAMES = new Set(["dist", "dist-runtime"]);
...
function resolveWorkspaceAcpxPluginRoot(currentRoot: string): string | null {
  if (
    ...
    BUNDLED_RUNTIME_DIR_NAMES.has(path.basename(path.dirname(path.dirname(currentRoot))))
  ) {
    const workspaceRoot = path.resolve(currentRoot, "..", "..", "..", "extensions", "acpx");
    return isAcpxPluginRoot(workspaceRoot) ? workspaceRoot : null;
  }

  if (!isAcpxPluginRoot(currentRoot)) {
    const siblingWorkspaceRoot = path.join(currentRoot, "extensions", "acpx");
    if (isAcpxPluginRoot(siblingWorkspaceRoot)) {
      return siblingWorkspaceRoot;
    }
  }
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  // In a live repo checkout, dist/ or dist-runtime/ can be rebuilt out from under the
  // running gateway. Prefer the stable source plugin root when a built extension is
  // running beside it.
...
function resolveOpenClawRoot(currentRoot: string): string {
  if (
    ...
    if (BUNDLED_RUNTIME_DIR_NAMES.has(path.basename(parent))) {
