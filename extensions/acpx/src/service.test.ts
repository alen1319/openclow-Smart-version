...
describe("createAcpxRuntimeService", () => {
  ...
  it("passes an explicit moduleUrl through plugin config resolution", async () => {
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "acpx-service-module-url-"));
    const pluginRoot = path.join(repoRoot, "extensions", "acpx");
    const moduleUrl = new URL(`file://${path.join(pluginRoot, "src", "config.ts")}`).href;
    const pluginToolsEntry = path.join(repoRoot, "src", "mcp", "plugin-tools-serve.ts");
    const { runtime } = createRuntimeStub(true);
    const runtimeFactory = vi.fn(() => runtime);
    const service = createAcpxRuntimeService({
      moduleUrl,
    ...
    const context = createServiceContext({ workspaceDir: repoRoot });

    try {
      fs.mkdirSync(path.join(pluginRoot, "src"), { recursive: true });
      fs.mkdirSync(path.dirname(pluginToolsEntry), { recursive: true });
      fs.writeFileSync(path.join(pluginRoot, "package.json"), JSON.stringify({ dependencies: { acpx: "0.4.1" } }), "utf8");
      fs.writeFileSync(path.join(pluginRoot, "openclaw.plugin.json"), "{}\n", "utf8");
      fs.writeFileSync(path.join(pluginRoot, "src", "config.ts"), "// test\n", "utf8");
      fs.writeFileSync(pluginToolsEntry, "// test\n", "utf8");
          ...
          pluginConfig: expect.objectContaining({
            mcpServers: expect.objectContaining({
              "openclaw-plugin-tools": {
                ...
                args: ["--import", "tsx", pluginToolsEntry],
              },
            }),
          }),
        }),
