...
describe("acpx plugin config parsing", () => {
  ...
  it("prefers the workspace plugin root for dist-runtime plugin bundles", () => {
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "acpx-root-dist-runtime-"));
    const workspacePluginRoot = bundledPluginRootAt(repoRoot, "acpx");
    const bundledRuntimePluginRoot = path.join(repoRoot, "dist-runtime", "extensions", "acpx");
    try {
      fs.mkdirSync(workspacePluginRoot, { recursive: true });
      fs.mkdirSync(bundledRuntimePluginRoot, { recursive: true });
      fs.writeFileSync(path.join(workspacePluginRoot, "package.json"), "{}\n", "utf8");
      fs.writeFileSync(path.join(workspacePluginRoot, "openclaw.plugin.json"), "{}\n", "utf8");
      fs.writeFileSync(path.join(bundledRuntimePluginRoot, "package.json"), "{}\n", "utf8");
      fs.writeFileSync(path.join(bundledRuntimePluginRoot, "openclaw.plugin.json"), "{}\n", "utf8");

      const moduleUrl = pathToFileURL(path.join(bundledRuntimePluginRoot, "index.js")).href;
      expect(resolveAcpxPluginRoot(moduleUrl)).toBe(workspacePluginRoot);
    } finally {
      fs.rmSync(repoRoot, { recursive: true, force: true });
    }
  ...
  it("resolves the workspace plugin root from a shared dist chunk at the repo root", () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(workspacePluginRoot, "package.json"), "{}\n", "utf8");
      fs.writeFileSync(path.join(workspacePluginRoot, "openclaw.plugin.json"), "{}\n", "utf8");
      fs.writeFileSync(path.join(repoRoot, "dist", "register.runtime-test.js"), "// test\n", "utf8");

      const moduleUrl = pathToFileURL(path.join(repoRoot, "dist", "register.runtime-test.js")).href;
      expect(resolveAcpxPluginRoot(moduleUrl)).toBe(workspacePluginRoot);
    } finally {
      fs.rmSync(repoRoot, { recursive: true, force: true });
    }
