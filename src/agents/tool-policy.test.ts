...
import {
  ...
  expandToolGroups,
  isAuthorizationLevelAllowed,
  isOwnerOnlyToolName,
  normalizeToolName,
  resolveToolAuthorizationContext,
  resolveToolAuthorizationRequirement,
...
describe("tool-policy", () => {
  ...
  it("defaults to guest when sender authorization is omitted", () => {
    expect(resolveToolAuthorizationContext().level).toBe("guest");
    expect(resolveToolAuthorizationContext({ senderIsOwner: false }).level).toBe("guest");
    expect(resolveToolAuthorizationContext({ senderIsApprover: false }).level).toBe("guest");
  });

  it("keeps backward compatibility for explicit levels when role signals are absent", () => {
    expect(resolveToolAuthorizationContext({ level: "allowed" })).toEqual({
      level: "allowed",
      senderIsOwner: false,
      senderIsApprover: false,
      isAuthorizedSender: true,
    });
    expect(resolveToolAuthorizationContext({ level: "owner" })).toEqual({
      level: "owner",
      senderIsOwner: true,
  ...
  it("does not allow explicit level to elevate beyond explicit role signals", () => {
    expect(
      resolveToolAuthorizationContext({
        level: "owner",
        senderIsOwner: false,
  ...
  it("keeps legacy owner-only filtering compatible for non-owner allowed senders", () => {
  ...
  it("resolves shared authorization requirements for privileged tools", () => {
    expect(resolveToolAuthorizationRequirement("sessions_spawn")).toEqual({
      minimumAuthorizationLevel: "allowed",
      approvalClass: "exec_capable",
    });
    expect(resolveToolAuthorizationRequirement("gateway")).toEqual({
      minimumAuthorizationLevel: "owner",
      approvalClass: "control_plane",
    });
    expect(resolveToolAuthorizationRequirement("read")).toEqual({
      minimumAuthorizationLevel: "guest",
    });
  });

  it("never relaxes named privileged tool requirements via per-tool metadata", () => {
    expect(
      resolveToolAuthorizationRequirement({
        name: "gateway",
        minimumAuthorizationLevel: "allowed",
      } as unknown as Pick<AnyAgentTool, "name" | "ownerOnly" | "minimumAuthorizationLevel">),
    ).toEqual({
      minimumAuthorizationLevel: "owner",
      approvalClass: "control_plane",
    });
  ...
  it("treats ownerOnly as a strict floor even when minimumAuthorizationLevel is lower", () => {
    expect(
      resolveToolAuthorizationRequirement({
        name: "custom_spawn_like_tool",
        ownerOnly: true,
        minimumAuthorizationLevel: "allowed",
      } as unknown as Pick<AnyAgentTool, "name" | "ownerOnly" | "minimumAuthorizationLevel">),
    ).toEqual({
  ...
  it("allows approvers to use allowed-level tools without escalating to owner", () => {
    const approver = resolveToolAuthorizationContext({ senderIsApprover: true });
    expect(approver.level).toBe("approver");
    expect(isAuthorizationLevelAllowed("allowed", approver)).toBe(true);
    expect(isAuthorizationLevelAllowed("owner", approver)).toBe(false);
  });
  ...
  it("keeps privileged tool boundaries stable across guest/allowed/approver/owner levels", () => {
    const tools = [
      { name: "sessions_spawn" },
      { name: "gateway" },
      { name: "nodes" },
    ...
    const guest = applyToolAuthorizationPolicy(tools, { level: "guest" }).map((tool) => tool.name);
    const allowed = applyToolAuthorizationPolicy(tools, { level: "allowed" }).map(
      (tool) => tool.name,
    );
    const approver = applyToolAuthorizationPolicy(tools, { level: "approver" }).map(
      (tool) => tool.name,
    );
    const owner = applyToolAuthorizationPolicy(tools, { level: "owner" }).map((tool) => tool.name);

    expect(guest).toEqual([]);
    expect(allowed).toEqual(["sessions_spawn"]);
    expect(approver).toEqual(["sessions_spawn"]);
    expect(owner).toEqual(["sessions_spawn", "gateway", "nodes", "cron"]);
  });
