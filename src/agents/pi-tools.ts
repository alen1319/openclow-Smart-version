...
import type { ToolAuthorizationLevel } from "./tools/common.js";
...
export function createOpenClawCodingTools(options?: {
  ...
  /** Whether the sender is an approver for approval-scoped capabilities. */
  senderIsApprover?: boolean;
  /** Effective authorization level for shared tool-policy gating. */
  senderAuthorizationLevel?: ToolAuthorizationLevel;
  /** Whether the sender passed channel/command authorization checks. */
  isAuthorizedSender?: boolean;
  /** Stable sender-scoped authorization subject key for audit trails. */
  authorizationSubjectKey?: string;
  /** Stable approver identity key for approval-chain audit trails. */
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...createOpenClawTools({
      ...
      authorizationSubjectKey: options?.authorizationSubjectKey,
      approverIdentityKey: options?.approverIdentityKey,
      senderIsOwner: options?.senderIsOwner,
      senderIsApprover: options?.senderIsApprover,
      senderAuthorizationLevel: options?.senderAuthorizationLevel,
  ...
  const authorizationContext = resolveToolAuthorizationContext({
    level: options?.senderAuthorizationLevel,
    senderIsOwner: options?.senderIsOwner,
    senderIsApprover: options?.senderIsApprover,
    isAuthorizedSender: options?.isAuthorizedSender,
  });
  const toolsByAuthorization = applyToolAuthorizationPolicy(
