...
describe("ensureAgentWorkspace", () => {
  ...
  it("loads the claude-code workspace preset when requested", async () => {
    const tempDir = await makeTempWorkspace("openclaw-workspace-");
    ...
    const agentsContent = await fs.readFile(path.join(tempDir, DEFAULT_AGENTS_FILENAME), "utf-8");
    expect(agentsContent).toContain("Agent-First Execution");
  });
});
