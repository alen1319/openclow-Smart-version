...
async function loadTemplate(name: string, preset?: string): Promise<string> {
  const normalizedPreset = normalizeWorkspaceTemplatePreset(preset);
  const cacheKey = `${normalizedPreset}:${name}`;
  ...
  const pending = (async () => {
    const templatePath = await resolveWorkspaceTemplatePath(name, { preset: normalizedPreset });
    try {
...
export async function ensureAgentWorkspace(params?: {
  ...
  const templatePreset = normalizeWorkspaceTemplatePreset(params?.templatePreset);
  ...
  const agentsTemplate = await loadTemplate(DEFAULT_AGENTS_FILENAME, templatePreset);
  const soulTemplate = await loadTemplate(DEFAULT_SOUL_FILENAME, templatePreset);
  const toolsTemplate = await loadTemplate(DEFAULT_TOOLS_FILENAME, templatePreset);
  const identityTemplate = await loadTemplate(DEFAULT_IDENTITY_FILENAME, templatePreset);
  const userTemplate = await loadTemplate(DEFAULT_USER_FILENAME, templatePreset);
  const heartbeatTemplate = await loadTemplate(DEFAULT_HEARTBEAT_FILENAME, templatePreset);
  ...
  if (!state.bootstrapSeededAt && !state.setupCompletedAt && !bootstrapExists) {
    ...
    if (legacySetupCompleted) {
      ...
      const bootstrapTemplate = await loadTemplate(DEFAULT_BOOTSTRAP_FILENAME, templatePreset);
