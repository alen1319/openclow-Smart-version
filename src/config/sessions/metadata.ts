...
export function deriveSessionMetaPatch(params: {
  ...
  const parentSessionKey = params.ctx.ParentSessionKey?.trim();
  if (parentSessionKey) {
    patch.parentSessionKey = parentSessionKey;
  }
  const memoryRootSessionKey = params.ctx.MemoryRootSessionKey?.trim();
  if (memoryRootSessionKey) {
    patch.memoryRootSessionKey = memoryRootSessionKey;
  }
  const authorizationSubjectKey = params.ctx.AuthorizationSubjectKey?.trim();
  if (authorizationSubjectKey) {
    patch.authorizationSubjectKey = authorizationSubjectKey;
  }
