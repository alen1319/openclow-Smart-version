...
export function resolveCommandAuthorization(params: {
  ...
  const senderIsApproverByScope =
    isInternalMessageChannel(ctx.Provider) &&
    Array.isArray(ctx.GatewayClientScopes) &&
    (ctx.GatewayClientScopes.includes("operator.approvals") ||
      ctx.GatewayClientScopes.includes("operator.admin"));
  const ownerAllowlistConfigured = ownerAllowAll || explicitOwners.length > 0;
  const senderIsOwner = ctx.ForceSenderIsOwnerFalse
    ? false
    : senderIsOwnerByIdentity || senderIsOwnerByScope || ownerAllowAll;
  const senderIsApprover = ctx.ForceSenderIsOwnerFalse
    ? false
    : senderIsOwner || senderIsApproverByScope;
  ...
  const senderAuthorizationLevel: ToolAuthorizationLevel = resolveToolAuthorizationContext({
    senderIsOwner,
    senderIsApprover,
    isAuthorizedSender,
  }).level;
