...
describe("OpenAI-compatible HTTP API (e2e)", () => {
  ...
  it("handles request validation and routing", async () => {
    ...
    try {
      ...
      {
        mockAgentOnce([{ text: "approver path ok" }]);
        const res = await postChatCompletions(
          port,
          {
            model: "openclaw",
        ...
        expect(getFirstAgentCall()).toEqual(
      ...
      {
        mockAgentOnce([{ text: "allowed path ok" }]);
        const res = await postChatCompletions(
          port,
          {
            model: "openclaw",
  ...
  it("treats shared-secret bearer callers as owner operators", async () => {
    ...
    try {
      ...
      expect(firstCall?.senderIsApprover).toBe(true);
      expect(firstCall?.senderAuthorizationLevel).toBe("owner");
      expect(firstCall?.isAuthorizedSender).toBe(true);
      await res.text();
    } finally {
  ...
  it("aborts agent command when non-streaming client disconnects", { timeout: 15_000 }, async () => {
    const port = enabledPort;
    let serverAbortSignal: AbortSignal | undefined;
