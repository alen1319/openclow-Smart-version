...
describe("POST /tools/invoke", () => {
  ...
  it("uses approver-level HTTP authorization for allowed-tier tools without granting owner-only tools", async () => {
    setMainAllowedTools({
      allow: ["sessions_spawn", "owner_only_test"],
      gatewayAllow: ["sessions_spawn"],
    });
    ...
    const spawnRes = await invokeTool({
      port: sharedPort,
      headers: approverHeaders,
      tool: "sessions_spawn",
      sessionKey: "main",
    });
    const spawnBody = await expectOkInvokeResponse(spawnRes);
    expect(spawnBody.result).toEqual({
      ok: true,
      route: { agentTo: undefined, agentThreadId: undefined },
    });
    expect(lastCreateOpenClawToolsContext).toEqual(
      expect.objectContaining({
        senderIsOwner: false,
    ...
    const ownerOnlyRes = await invokeTool({
      port: sharedPort,
      headers: approverHeaders,
      tool: "owner_only_test",
      sessionKey: "main",
    });
    expect(ownerOnlyRes.status).toBe(404);
  });

  it("propagates HTTP authorization subject identity to tool context for approver calls", async () => {
    setMainAllowedTools({
  ...
  it("does not propagate approverIdentityKey for non-approver HTTP callers", async () => {
    setMainAllowedTools({
      allow: ["sessions_spawn"],
      gatewayAllow: ["sessions_spawn"],
    });
